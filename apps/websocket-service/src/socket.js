...
module.exports = {
  ...
  // TMI runs immediately AFTER the radar broadcast each minute: the engine must see the
  // snapshot the radar just acted on, never the previous one.
  broadcastTmi: (date) => broadcastTmi(io, date),
  emitRadarNew: (date, symbols) => emitRadarNew(io, date, symbols),
  allowedOrigins,
};